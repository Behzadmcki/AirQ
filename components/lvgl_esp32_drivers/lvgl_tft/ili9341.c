...
{
	ESP_LOGI(TAG, "Before lcd inti cmd.");
	...
	ESP_LOGI(TAG, "after lcd inti cmd.");
	...
	ESP_LOGI(TAG, "midle of Initialize non-SPI GPIOs");
	gpio_reset_pin(ILI9341_RST);
	gpio_set_direction(ILI9341_RST, GPIO_MODE_OUTPUT);

	ESP_LOGI(TAG, "Initialize non-SPI GPIOs");

#if ILI9341_ENABLE_BACKLIGHT_CONTROL
	gpio_config(ILI9341_BCKL);
